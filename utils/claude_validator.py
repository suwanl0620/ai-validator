...
"""
    ...
    def get_rules_summary(self, rules_text: str) -> Dict[str, Any]:
        """
        Use Claude to analyze and summarize the rules document.
        """
        summary_prompt = f"""
Analyze the following EXIM Bank rules document and provide a structured summary:
...
{{
    "document_type": "type of document (e.g., 'EXIM Bank Insurance Claim Guidelines', 'Application Requirements')",
    "key_sections": [
        {{
            "section_name": "name of section",
            "requirements_count": number,
            "critical_requirements": ["list of most important requirements in this section"]
        }}
    ],
    "required_documents": ["comprehensive list of all required documents"],
    "required_information": ["comprehensive list of all required information/fields"],
    "deadlines_and_timeframes": ["any time-sensitive requirements"],
    "approval_criteria": ["key criteria that determine approval/rejection"],
    "common_rejection_reasons": ["likely reasons applications get rejected based on these rules"],
    "exim_specific_requirements": ["requirements specific to EXIM Bank procedures"]
}}

Focus particularly on EXIM Bank specific procedures, requirements, and guidelines.
Provide your response as valid JSON only.
"""

        try:
            messages = [{"role": "user", "content": summary_prompt}]
            response_text = self._invoke_claude(messages, max_tokens=2000, temperature=0.1)
            
            return json.loads(response_text.strip())
            
        except Exception as e:
            return {"error": f"Failed to summarize rules: {str(e)}"}
    ...
    def validate_specific_requirements(
        ...
        """
        Check specific requirements against the application.
        """
        requirements_text = "\n".join([f"- {req}" for req in specific_requirements])
        
        prompt = f"""
Check if the following specific requirements are met in the submitted EXIM Bank application:
...
For each requirement, respond with one of: MET, FAILED, UNCLEAR, NOT_APPLICABLE
Provide a brief explanation for each status.

Provide your response as JSON in this format:
{{
    "requirement_1": "MET - brief explanation of how it's met",
    "requirement_2": "FAILED - brief explanation of why it failed",
    "requirement_3": "UNCLEAR - brief explanation of what's unclear"
}}

Use the exact requirement text as the key in your JSON response.
"""

        try:
            messages = [{"role": "user", "content": prompt}]
            response_text = self._invoke_claude(messages, max_tokens=1500, temperature=0.1)
            
            return json.loads(response_text.strip())
            
        except Exception as e:
            return {"error": f"Failed to validate specific requirements: {str(e)}"}
    
    def extract_key_information(self, application_text: str) -> Dict[str, Any]:
        """
        Extract key information from the application for quick review.
        """
        extraction_prompt = f"""
Extract key information from this EXIM Bank application:
...
{{
    "applicant_information": {{
        ...
        "contact_person": "extracted contact person or null",
        "address": "extracted address or null",
        "phone": "extracted phone or null",
        "email": "extracted email or null"
    }},
    "claim_details": {{
        "claim_amount": "extracted claim amount or null",
        "policy_number": "extracted policy number or null",
        "claim_date": "extracted claim date or null",
        "incident_date": "extracted incident date or null",
        "claim_type": "type of claim or null"
    }},
    "documents_referenced": ["list of documents mentioned in the application"],
    "signatures_present": true or false,
    "dates_mentioned": ["list of all dates found in the document"],
    "critical_information_missing": ["list of critical info that appears to be missing"]
}}
